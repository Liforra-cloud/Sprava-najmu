...
export default function SidebarLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      {/* Sidebar */}
      <div className={`fixed z-20 inset-y-0 left-0 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-200 ease-in-out bg-slate-800 w-36 text-white`}>
        <div className="flex items-center justify-between md:justify-center px-4 py-4 border-b border-slate-700">
          <span className="text-base font-semibold text-center">Správa</span>
        ...
        <nav className="mt-4 space-y-1">
          {navItems.map(({ href, label, icon }) => (
            <Link
              key={href}
              href={href}
              className={`flex items-center gap-2 px-3 py-2 transition-all duration-150 rounded-r-md border-l-4 text-sm ${
                pathname === href
          ...
          {/* Účetní sekce */}
          <p className="text-xs text-slate-400 mt-6 mb-1 px-3 uppercase tracking-wide">Účet</p>
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 mt-1 text-slate-300 hover:bg-slate-700 hover:text-white transition-all duration-150 w-full text-left text-sm"
          >
            <LogOut size={18} />
            <span>Odhlásit</span>
          </button>
          ...
          {/* Přihlášený uživatel */}
          {userEmail && (
            <div className="text-xs text-slate-400 px-3 mt-6">
              Přihlášen jako:
              <br />
      ...
      {/* Obsah */}
      <div className="flex-1 flex flex-col ml-0 md:ml-36">
        {/* Topbar (mobil) */}
