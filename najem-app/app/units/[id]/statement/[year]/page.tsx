...
// Dummy data pro ukázku (v praxi místo toho načteš součty za období!)
const exampleData = [
...
export default function StatementTable() {
  // Na začátku načti všechny účtované položky s jejich součtem záloh
  ...
  // Seznam všech možných položek, které ještě nejsou přidané
  const unusedItems = PREDEFINED_ITEMS.filter(
    i => !items.some(used => used.id === i.id)
  );

  // Přidání nové položky (ze seznamu nebo úplně nová)
  const addItem = (itemId?: string) => {
    let base = { id: generateId(), name: '', totalAdvance: 0, consumption: '', unit: '', totalCost: '', diff: 0 };
    if (itemId) {
